...
%for j=1:100
    shortest_path(T)
    %load('OptimalPath'); %quello che viene salvato da "shortest_path.m
    load('fin');
    for l=1:size(fin, 1)
        alfaAngle(l, :)=0;
        betaAngle(l, :)=0;
    ...
    for i=1:1:size(fin, 1)
        ...
        puntifin=fin;
         driver      		= SimulationDriver();
          ...
          inizio = [puntini(i, 2)*scaleY, -puntini(i, 1)*scaleX, maxZ*scaleZ-0.1];
          fine   = [puntifin(i, 2)*scaleY,  -puntifin(i, 1)*scaleX, maxZ*scaleZ-0.1];
