...
a=find(DATI(:, 1)==1);
tutti_uni=DATI(a, :);
tutti_uni=tutti_uni(1: 1083, :);

DATI=[tutti_uni; tutti_zeri];

DATI=datasample(DATI, 1083*2);
...
DATI_TEST=totalepatternmediati;
...
NET=newff(P_LEARNING_INPUT', P_LEARNING_OUTPUT', 30);
% clc
i
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for j=1:size(TSIMULATED, 2)
    if (TSIMULATED(j)>0.5)
        ...
        TSIMULATED2(j)=100000; %non ce la fa
    end
end
...
totale_patteren_input_e_output=[totalepatternmediati tmp1];

save('totale_patteren_input_e_output');
...
% a=find(DATI(:, 1)==1);
% tutti_uni=DATI(a, :);
% tutti_uni=tutti_uni(1: 681, :);
% 
% DATI=[tutti_uni; tutti_zeri];
% 
% DATI=datasample(DATI, 1365);
% new2=zscore(DATI(:, 4)); %pendenza partenza
% new3=zscore(DATI(:, 5)); %pendenza arrivo
% new4=zscore(DATI(:, 6)); %Hmax partenza
% new5=zscore(DATI(:, 7)); %Hmax arrivo
...
% % new67=(new6+new7)/2;
% 
% DATI=[new1 new2 new4 new6 output new919];
% tmp1=0;
% tmp0=0;
% tmp=0;
% 
...
% P_TEST_INPUT=DATI_TEST(:, 1:4);
% 
% NET=newff(P_LEARNING_INPUT', P_LEARNING_OUTPUT', 21);
% clc
% i
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   NET.trainParam.epochs=10000
