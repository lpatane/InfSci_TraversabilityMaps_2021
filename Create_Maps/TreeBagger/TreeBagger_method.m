...
for i=1:100
    k=1;
   BaggedEnsemble = TreeBagger(i,Data.input,Data.output,'OOBPred','On','Method', 'classification', 'MaxNumSplits', 1000)
...
%     fin=1-rsLoss;
    ...
    %B=str2double(B);
    cm = confusionmat(output, pred_model2r_oobY1)
    ...
    sum(cm);
    perc=cm/sum(ans);
    accuracy(i, 1)=perc(1, 1)+perc(2, 2);
