...
for i=1:size(DATI, 1)-1
    for j=i+1:size(DATI, 1)-1
        if ( (abs(DATI(i, 1)-DATI(j, 1))<e && abs(DATI(i, 2)-DATI(j, 2))<e && abs(DATI(i, 3)-DATI(j, 3))<e && abs(DATI(i, 4)-DATI(j, 4)<e)) && ((DATI(i, 5)==1 && DATI(j, 5)==0) ||(DATI(i, 5)==0 && DATI(j, 5)==1)))
            elimina(k, 1)=j;
            elimina(k, 2)=i;            
            k=k+1;
            numero=numero+1;
